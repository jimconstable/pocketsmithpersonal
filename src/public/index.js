...
let updateChart = function() {
  let chartData = d3.select("#cumulative").property("checked")
    ? dataAcc
    : dataPer;
  updateChart1(chartData);
  let diffData = d3.select("#cumulative").property("checked")
    ? diffAcc
    : diffPer;
...
d3.json("/totals/")
  .then(trends => {
    ...
    trends.forEach((m, i, a) => {
        ...
        if (k != "start_date") {
          if (
            ...
            output.accum.push({
              ...
              value:
                m[k] +
                (i > 0 ? _.findLast(output.accum, e => e.type == k).value : 0)
            });
          }
        }
      });
      if (
        start_date < new Date() &&
        new Date() < d3.timeMonth.offset(start_date, 1)
      ) {
      ...
      if (start_date < d3.timeMonth.offset(new Date(), 1)) {
        output.diffPeriod.push({
          start_date,
          type: "actual",
          value: m.income_actual - m.expense_actual
        });
        output.diffAccum.push({
          start_date,
          type: "actual",
          value:
            m.income_actual -
            m.expense_actual +
            (i > 0
              ? _.findLast(output.diffAccum, e => e.type == "actual").value
              : 0)
        });
      }
      output.diffPeriod.push({
      ...
      output.diffAccum.push({
        ...
        value:
          ...
          m.expense_forecast +
          (i > 0
            ? _.findLast(output.diffAccum, e => e.type == "forecast").value
            : 0)
      });
      if (
        start_date < new Date() &&
...
function parseDate(d) {
  return d3.timeMonth.offset(d3.utcParse("%Y-%m-%d")(d), 1);
}
